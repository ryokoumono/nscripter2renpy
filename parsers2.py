...
class Parser(object):
    def __init__(self):
        ...
        self.rules = [
            ...
            ("JUMP", (r"jumpb|jumpf", self._jump)),
            ("JUMP_TO", (r"^~", self._jump_to)),
    ...
    def _list(self, scanner, token, line):
        return token.replace('?', '')
    def _jump(self, scanner, token, line):
        self.jumps.append(True)
        return len(self.jumps) - 1

    def _jump_to(self, scanner, token, line):
        if not line in self.jump_to:
            self.jump_to[line] = "__jump__%i" % self.njump
            self.jump_to_list.append("__jump__%i" % self.njump)
            self.njump += 1
    ...
    def tokenize(self, content):
        ...
        self.tokens = [token for token in lex.scan(content) if token is not None]
...
class Translator(object):
        ...
        while True:
            ...
            for skipto in self.parser.skiplabel.keys():
                if not skipto in skipdone and skipto <= token.line:
                    ...
                    break
            for jumpto in self.parser.jump_to.keys():
                if not jumpto in skipdone and jumpto <= token.line:
                    self.write_statement('\nlabel %s:' % self.parser.jump_to[jumpto])
                    skipdone[jumpto] = True
                    break
    ...
    def handle_token(self, token):
        ...
        if token.type == "IDENTIFIER":
            ...
            comment = token.value.replace(';', '')
            self.write_statement('#%s' % comment)
        elif token.type == "JUMP_TO":
            pass
        elif token.type == "JUMP":
            self.read_jump(token)
        elif token.type == "VARSTR":
            self.read_var(token)
        else:
    ...
    def read_text(self, token):
        ...
        if '!w' in text:
            tmp_list = re.findall(r'!w\d+', text)
            tmp_list = list(dict.fromkeys(tmp_list))
            for x in tmp_list:
                var = '{w=%s}' % (float(re.search('\d+', x).group(0)) / 1000)
                text = re.sub(x, var, text)
        if '!s0' in text:
            text = re.sub('!s0', '', text) + '{fast}'
    ...
    def read_jump(self, token):
        self.write_statement('jump %s' % self.parser.jump_to_list[token.value])

    def read_var(self, token):
        self.write_statement('"[%s]"' % token.escaped)
    ...
    def read_command(self, token):
        if token.value == 'add':
            ...
            self.cmd_dim()
        elif token.value == 'rnd':
            self.cmd_rnd()
        elif token.value == 'movl':
            self.cmd_movl()
        elif token.value == 'itoa':
            self.cmd_itoa()
        else:
    ...
    def cmd_bg(self):
        #bg "image\word\type_moon.png",%type_crossfade_mid

        bg = self.parser.read(["STR", "COLOR", "VARSTR", "STRALIAS"])
        comma = self.parser.read("COMMA", mandatory=False)
        if comma is not None:
            effect = self.parser.read(["NUM", "VARNUM", "NUMALIAS"]).value
            self.write_statement('scene image %s with %s' % (bg.escaped, effect.replace('%','')))
        else: 
            self.write_statement('scene image %s' % bg.escaped)
    ...
    def cmd_cl(self):
        ...
        if pos == 'a':
            ...
            self.write_statement('$ renpy.with_statement(%s)' % effect.replace('%', ''))
    ...
    def cmd_date(self):
        ...
        self.write_statement('$ %s = int(datetime.date.today().year)' % (year.escaped))
        self.write_statement('$ %s = int(datetime.date.today().month)' % (month.escaped))
        self.write_statement('$ %s = int(datetime.date.today().day)' % (day.escaped))
    ...
    def cmd_gosub(self):
        # LABEL
        label = self.parser.read(["LABEL", "VARSTR"])
        if label.type == 'LABEL':
            self.write_statement('call %s' % label.value.replace('*', ''))
        elif label.type == 'VARSTR':
            self.write_statement('call expression %s' % label.escaped)        
    ...
    def cmd_goto(self):
        ...
        if label.type == 'LABEL':
            ...
            self.write_statement('jump expression %s' % label.escaped)
    ...
    def cmd_if(self, notif=False):
        ...
        while True:
            if self.parser.peek().value == 'fchk':
                ...
                tmp = ''
                op = self.parser.read(["NUM", "VARNUM", "NUMALIAS", "LT", "LE", "GT", "GE", "EQ", "NEQ", "AND", "OR", "LIST", "STR", "VARSTR"], mandatory=False)

                if op is None:
                    break

                elif op.type == "LIST":
                    self.parser.read("BRACKET")
                    tmp = self.parser.read("VARNUM")
                    self.parser.read("BRACKET")
                    op = '%s[%s]' % (op.escaped, tmp.escaped)
                    stmt += ' ' + op
        ...
        if notif:
            self.write_statement("if not (%s):" % stmt.replace('if', ''))
        else:
    ...
    def cmd_mov(self):
        ...
        list_true = False
        if var.type == "VARNUM":
            val = self.parser.read(["NUM", "VARNUM", "NUMALIAS", "LIST"])
            if val.type == "LIST":
                self.parser.read("BRACKET")
                tmp = self.parser.read("VARNUM")
                self.parser.read("BRACKET")
                val.value = '%s[%s]' % (val.escaped, tmp.escaped)
                list_true = True
        else:
            val = self.parser.read(["STR", "VARSTR", "STRALIAS"])
        if list_true:
            self.write_statement('$ %s=%s' % (var.escaped, val.value))
        else:
            self.write_statement('$ %s=%s' % (var.escaped, val.escaped)) 
    ...
    def cmd_dim(self):        
        name = self.parser.read("LIST")
        self.parser.read("BRACKET")
        num = self.parser.read("NUM")
        self.parser.read("BRACKET")
        self.write_statement('$ %s = []' % name.value)

    def cmd_rnd(self):
        name = self.parser.read("VARNUM")
        self.parser.read("COMMA", mandatory=False)
        _range =  self.parser.read("NUM")
        self.write_statement('$ %s = renpy.random.randint(0, %s)' % (name.escaped, _range.value))
    ...
    def cmd_movl(self):
        ...
        numbers = []
        while True:
            num = self.parser.read("NUM", mandatory=False)
            self.parser.read("COMMA", mandatory=False)

            if num is None:
                break
            else:
                numbers.append(int(num.value))
        
        self.write_statement('$ %s = %s' % (name.value, numbers))

    def cmd_itoa(self):
        var = self.parser.read("VARSTR")
        self.parser.read("COMMA", mandatory=False)
        num = self.parser.read(["VARNUM", "NUM"])
        self.write_statement('%s = str(%s)' % (var.value, num.escaped))
